...
uint32_t  THREADBPWORKLOAD = 72100000000000000;
...
uint64_t N = 0;
/*thuynote
uint64_t N_SEQUENT7IAL_MAX = 0x100000000;
uint64_t DEBUGCOUNT = 0x400; */
uint64_t N_SEQUENTIAL_MAX = 0x72100000000000000;
uint64_t DEBUGCOUNT = 0x1000000000
uint64_t u64range;
...
//thuynote uint64_t bsgs_m = 4194304;

uint64_t bsgs_m = 1000000000000;
uint64_t bsgs_m2;
uint64_t bsgs_m3;
...
int main(int argc, char **argv)	{
	...
	if(FLAGMODE != MODE_BSGS )	{
		if(FLAG_N){
			...
			if(N_SEQUENTIAL_MAX < 100024)	{
				fprintf(stderr,"[I] n value need to be equal or great than 1024, back to defaults\n");
				FLAG_N = 0;
				//thuynote N_SEQUENTIAL_MAX = 0x100000000;
				N_SEQUENTIAL_MAX = 0x72100000000000000;
			}
	...
	if(FLAGMODE == MODE_BSGS )	{
		...
		/* thuynote BSGS_N.SetInt64((uint64_t)0x100000000000); */
			BSGS_N.SetInt64((uint64_t)0x72100000000000000);
		}
	...
	M3	5497558139
		*/
		...
		// thuynote
		printf(": %.2f MB\n",(float)((float)(uint64_t)bloom_bP_totalbytes/(float)(uint64_t)72100000000000000));
		...
		printf(": %.2f MB\n",(float)((float)(uint64_t)bloom_bP2_totalbytes/(float)(uint64_t)72100000000000000));
		...
		printf(": %.2f MB\n",(float)((float)(uint64_t)bloom_bP3_totalbytes/(float)(uint64_t)72100000000000000));
...
bool readFileAddress(char *fileName)	{
	...
	*/
	...
	// thuynote
			printf("[+] Allocating memory for %" PRIu64 " elements: %.2f MB\n",N,(double)(((double) sizeof(struct address_value)*N)/(double)72100000000000000));
...
bool forceReadFileAddress(char *fileName)	{
	...
	// thuynote
	printf("[+] Allocating memory for %" PRIu64 " elements: %.2f MB\n",numberItems,(double)(((double) sizeof(struct address_value)*numberItems)/(double)72100000000000000));
...
bool initBloomFilter(struct bloom *bloom_arg,uint64_t items_bloom)	{
	...
	//thuynote
	printf("[+] Loading data to the bloomfilter total: %.2f MB\n",(double)(((double) bloom_arg->bytes)/(double)72100000000000000));
	return r;
}
